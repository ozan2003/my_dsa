...
void sift_down(std::vector<T>& items, const std::size_t index, const std::size_t end)
{
    ...
    // If the largest element is not the parent, swap the parent with the largest element.
    if (largest != index)
    {
        std::swap(items[index], items[largest]);
        sift_down(items, largest, end); // Percolate down from the largest.
    }
}
...
void heap_sort(std::vector<T>& items)
{
    ...
    // Sort the elements.
    for (int end{static_cast<int>(items.size() - 1)}; 0 <= end; --end)
    {
        std::swap(items[0], items[end]); // Move the largest element to the end.
        sift_down(items, 0, end);        // Move the new first element to its correct place in the heap
    }
}
#endif // HEAPSORT_HPP
